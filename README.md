...
これにより、固定したDの候補は楕円曲線の整数点計算を使わず有限還元できます。
T∣D−δ₁²δ₂も使い、約数・指数の列挙と整数の二分探索だけで候補を尽くす手続きを実装しました。
[証明と有限還元](i3_mixed_parameter_bound_2026-09-20.md)・
[再現用コード](audit_i3_mixed_parameter_bound.py)・
[検算結果](verification_i3_mixed_parameter_bound.json)。
Dがいくらでも増える場合は残り、i=3全体の解決ではありません。

```text
python -X utf8 audit_i3_mixed_parameter_bound.py
```

**2026-09-20 並行研究：ChatGPT側にもDの増大領域の研究を依頼しました。**
こちらでは、任意の固定された奇素数集合がC(n,3)から全て消える無限族と、
その全素数がC(n,3)に現れてもC(n,j)の全桁条件を通過する無限族を明示しました。
[証明と研究記録](i3_finite_prime_obstruction_2026-09-20.md)。
これらは反例ではなく、固定した素数だけを調べる方式の限界を示します。
Dの一様上限や増大領域の排除は今回も得られていません。

**2026-09-20 添付統合：新しい引き継ぎ資料を原本のまま取り込みました。**
大規模な再計算は行わず、主要な整数恒等式を既存の因子表示に接続しました。
三次成分の付値にはv=1の例外を補い、式が省略されている次数4〜6の報告は未確認と明記しています。
[統合と適用範囲](i3_handoff_uniform_integration_2026-09-20.md)・
[添付原本](source_progress_handoff_2026-09-20.md)。その先の進展は上記の混合した差の評価です。
