...
def replay(filename, primes_filename):
    data = json.loads(artifact_path(filename).read_text(encoding='utf-8'))
    cert = json.loads(artifact_path(primes_filename).read_text(encoding='utf-8'))
    proved = {2}
...
if __name__ == '__main__':
    ...
    artifact_path(args.output).write_text(json.dumps(results, indent=2), encoding='utf-8')
