...
def read_and_check_curves():
    records = json.loads(artifact_path('gap13_curve_cases.json').read_text(encoding='utf-8'))
    ...
    for batch in range(7):
        ...
        src = artifact_path(name).read_text(encoding='utf-8')
        ...
        text = '\n'.join(ET.parse(artifact_path(f'magma_gap13_batch{batch}.xml')).getroot().itertext())
...
def main():
    cert = json.loads(artifact_path('i3_gap13_certificate.json').read_text(encoding='utf-8'))
    ...
    artifact_path('verification_i3_gap13.json').write_text(json.dumps(result, indent=2) + '\n', encoding='utf-8')
