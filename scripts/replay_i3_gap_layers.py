...
def read_curves():
    ...
    rows=json.loads(artifact_path('gap_curve_cases.json').read_text(encoding='utf-8'))
    assert {r['a4'] for r in rows}==expected
    all_points={}
    for row in rows:
        a4=row['a4']
        src=artifact_path(row['input']).read_text(encoding='utf-8')
        ...
        transcript='\n'.join(ET.parse(artifact_path(row['response'])).getroot().itertext())
...
def main():
    check_small_gaps()
    cert=json.loads(artifact_path('i3_gap_certificate.json').read_text(encoding='utf-8'))
    ...
    artifact_path('verification_i3_gap_layers.json').write_text(json.dumps(result,indent=2)+'\n',encoding='utf-8')
