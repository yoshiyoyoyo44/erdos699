...
def main():
    ...
    for batch in range(7):
        ...
        artifact_path(name).write_text(code, encoding='utf-8')
        for row in subset:
            row.update(input=name, response=f'magma_gap13_batch{batch}.xml')
    artifact_path('gap13_curve_cases.json').write_text(json.dumps(curves, indent=2) + '\n', encoding='utf-8')
    cert = {'schema': 1, 'min_u': 49, 'gap_range': [10, 13], 'period': PERIOD,
            'moduli': moduli, 'cases': cases}
    artifact_path('i3_gap13_certificate.json').write_text(json.dumps(cert, indent=2) + '\n', encoding='utf-8')
