...
def main():
    ...
    for w,d1,d2 in CASES:
        ...
        Path(name).write_text(source(w,d1,d2),encoding='utf-8')
    ...
    Path('center_curve_cases.json').write_text(json.dumps(rows,indent=2)+'\n',encoding='utf-8')
    endpoint_rows=[]
    for lam,d1,d2 in ENDPOINT_CASES:
        name=f'i3_endpoint_l{lam}_d{d1}{d2}.magma'
        Path(name).write_text(endpoint_source(lam,d1,d2),encoding='utf-8')
        endpoint_rows.append(dict(lam=lam,delta1=d1,delta2=d2,input=name,
                                 response=f'magma_endpoint_l{lam}_d{d1}{d2}.xml'))
    Path('endpoint_curve_cases.json').write_text(json.dumps(endpoint_rows,indent=2)+'\n',encoding='utf-8')
