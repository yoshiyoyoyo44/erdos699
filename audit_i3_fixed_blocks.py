...
def transcript(path):
    ...
    if root.get('aggregate')=='true':
        ...
        source=Path(provenance.get('full_input'))
        ...
        for entry in provenance:
            ip,op=Path(entry.get('input')),Path(entry.get('response'))
...
def main():
    ...
    modular=json.loads(Path('fixed_block_modular_exclusions.json').read_text())
    report['modular_certificates']={}
    for name,certificates in modular.items():
        rows=json.loads(Path(f'small_{name}_cases.json').read_text())
    ...
    for path in sorted(Path('.').glob('small_*_cases.json')):
        match=re.fullmatch(r'small_([ABC])(\d+)_cases.json',path.name)
        if not match:
            continue
        role,value=match[1],int(match[2])
        xml=Path(f'magma_small_{role}{value}.xml')
        if not xml.exists():
            continue
        ...
        source=Path(f'i3_small_{role}{value}.magma').read_text()
        ...
        report['fixed_blocks'].append(dict(role=role,value=value,cases=results,magma_version=version,
                                           source_sha256=hashlib.sha256(Path(f'i3_small_{role}{value}.magma').read_bytes()).hexdigest(),
                                           transcript_sha256=hashlib.sha256(xml.read_bytes()).hexdigest()))
    Path('verification_i3_fixed_blocks.json').write_text(json.dumps(report,indent=2)+'\n',encoding='utf-8')
