...
def read_case(row):
    ...
    input_bytes=Path(row['input']).read_bytes()
    assert input_bytes.decode('utf-8').replace('\r\n','\n')==source(w,d1,d2)
    response=Path(row['response']).read_bytes()
...
def read_endpoint(row):
    ...
    raw_input=Path(row['input']).read_bytes()
    assert raw_input.decode('utf-8').replace('\r\n','\n')==endpoint_source(lam,d1,d2)
    raw_response=Path(row['response']).read_bytes()
...
def main():
    ...
    rows=json.loads(Path('center_curve_cases.json').read_text())
    endpoint_rows=json.loads(Path('endpoint_curve_cases.json').read_text())
    ...
    Path('verification_i3_center_curve.json').write_text(json.dumps(report,indent=2)+'\n',encoding='utf-8')
