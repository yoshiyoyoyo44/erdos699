...
def main():
    ...
    modular=(json.loads(Path('fixed_block_modular_exclusions.json').read_text()).get(f'{role}{value}',{})
             if args.modular else {})
    Path(f'small_{role}{value}_cases.json').write_text(json.dumps(rows,indent=2)+'\n',encoding='utf-8')
    ...
    Path(f'i3_small_{role}{value}.magma').write_text('\n'.join(lines)+'\n',encoding='utf-8')
